 ...
 * 
 * 객체.inputValue("새우깡", 100); ==> 자동으로 2번 호출
...
public class InputServiceV2 {
	...
	/*
	 * 정수 범위에 대한 유효성 검사를 수행하는 메서드 
	 * 이미 만들어진 inputValue(String title) 메서드와 연동하여 사용하겠다~!
	 */
	public Integer inputValue(String title, int start, int end) {
		...
		while (true) {
			Integer intNum = this.inputValue(msgTitle);
			// this.inputValue(msgTitle);메서드가 리턴해준 값이
			// null이 아니면 유효성 검사 수행
			if (intNum != null) {
				// 유효성 검사를 통과하지 못하면
				if (intNum < start || intNum > end) {
					// 메시지를 보여주고
					System.out.printf("%d ~ %d의 값을 입력해야합니다\n", start, end);
					// 다시 입력받기
					continue;
				}
			}
			// .inputValue(msgTitle)가 null을 리턴했거나
			// 유효한 범위의 정수를 리턴했으면
			// 그값이 담겨있다. 그 값을 리턴하라
			return intNum;
		}
	...
	public Integer inputValue(String title) {
		...
		/*
		 * exception이 발생했을 때 사용자에게 안내 메시지를 보여주고 
		 * 다시 입력받기 위한 선조치
		 */
		while (true) {
			...
			// try-catch 블럭의 코드가 정상적으로 모두 완료되었을 때 실행되는 부분
