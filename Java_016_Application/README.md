...
* 데이터를 입력하여 어떤 연산을 수행할 때, 연산을 수행할 때 마다 데이터를 입력하여 처리한다면 매우 불편할 것이다.
...
1. Text 파일과 BIN(Binary, 이진)파일
2. Text 파일은 윈도우의 메모장, Sublime Text와 같은 도구를 사용하여 파일을 열었을 때 내용을 어려움 없이 읽을 수 있는 파일
3. BIN(Binary, 이진)파일은 이미지, 사진, 동영상, 음악, HWL, XLS, PPT 등의 파일들을 메모장으로 열었을 때 내용 깨짐 현상이 있어 정확히 읽을 수 없는 파일
4. Text 파일은 내용이ㅣ ASCII 코드 값으로 변환되어 저장되는 파일
5. BIN(Binary, 이진)파일은 파일마다 독특한 방법으로 저장이 되고 모든 내용이 이진수들의 나열로 저장된다.
6. FileWriter, PrintWriter는 Text파일을 만들고 Text방식으로 저장하는데 사용하는 클래스다
7. FlieWriter, BufferedReader는 Text파일을 읽어서 컴퓨터 메모리로 가져올 때 사용하는 클래스다